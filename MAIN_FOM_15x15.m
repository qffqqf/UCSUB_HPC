...
reduction_I_tf = 0; % 0 or 1; Interior modal reduction [BMS]
reduction_A_tf = 0; % 0 or 1; Boundary modal reduction [GBMS] (only 1 if reduction_I_tf = 1)
...
n_modes_I = 20; % Number of interior fixed-interface mode shapes [BMS] (choose (e.g. 2x max freq of interest), but always inspect results): off-line pretest available
n_modes_A = 40; % Number of boundary mode shapes/boundary [GBMS] (choose (e.g. 3x interior modes), but always inspect results): off-line pretest available
damping = 1e-2;

%% Final assembly MOR
reduction_T_tf = 0; % Final global modal reduction of the assembled system
n_modes_T = 200; % Number of global modes
...
force_UC_x = 6; % Which UC in the x-direction 12
force_UC_y = 6; % Which UC in the y-direction 9
...
% figure
% eigval = eigs(K_UC_redI, M_UC_redI, 10, 'smallestabs');
% plot(abs(eigval));
...
if reduction_I_tf||reduction_A_tf
    ...
    uz = output_selection*V*B_connectivity*Response_connected;
else
    uz = output_selection*B_connectivity*Response_connected;
end
...
%% SAVING RESULTS %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
save([savename,'.mat'],'freq','uz_fom','timing')
