...
BC = 0; % fixed boundary dofs = 1, free boundary dofs = 0
...
force_UC_x = 5; % Which UC in the x-direction 12
force_UC_y = 5; % Which UC in the y-direction 9
...
response_UC_x = 5; % Which UC in the x-direction
response_UC_y = 5; % Which UC in the y-direction
...
if force_UC_node == 'all'
    ...
    alldofs_disconnected_R = (allnodes_disconnected_response-1)*n_dofnodes+z_id;
...
if reduction_I_tf
    ...
    for i=1:length(idx_UCs_response(:))
        input_UC_selector(idx_UCs_response(i),idx_UCs_response(i))=1;
    end
...
if solve_frf_tf
    if reduction_T_tf
        disp('Calculating FRF ...')
        uz_gbms = SOAR(K_connected, M_connected, F_connected, R_connected, 1000, freq, 1e-5);
    end
end
